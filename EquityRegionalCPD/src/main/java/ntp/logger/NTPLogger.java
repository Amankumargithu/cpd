...
public class NTPLogger {
	...
	{
		info("SUBSCRIBE " + ticker + " on "+ streamName + " streamId " + streamId);
	}
	...
	{
		info("IMAGE " + image.tkr() + " on " + channelName + " - " + image._trdPrc + "," + image._ask + "," + image._bid + "," + image.protocol());
	}
	public static void dead(String ticker, String channelName)
	{
		info("DEAD " + ticker + " on " + channelName);
	}
	...
	{
		info("NEWISSUE " + ticker + " " + instrument);
	}
	...
	{
		info("UNSUBSCRIBE " + ticker + " on streamID " + streamId);
	}
