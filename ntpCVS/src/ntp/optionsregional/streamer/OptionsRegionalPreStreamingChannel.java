...
public class OptionsRegionalPreStreamingChannel extends UltraChan {
	...
	public OptionsRegionalPreStreamingChannel(String name, String ip, int port) {
	...
	public void OnUpdate(String StreamName, OPTrade trade) {
		...
		NTPLogger.info("PREREG_TRADE " + ticker + " " + trade._trdPrc + " " + trade._trdVol + " " + trade.pDateTimeMs() );
		QTCPDMessageBean qtMessageBean =  OptionsRegionalQTMessageQueue.getInstance().getCachedBean(ticker);
		...
		try
		{
			if(blobTable != null)
			{
				...
				for (int count = 0; count < rowCount; count++)
				{
					...
					if( !(ticker.startsWith("P:") && instrument.startsWith("OPR")) )
					{
					...
					if(ticker.contains("/") ){
						NTPLogger.newIssue(ticker, instrument);
						String opticker = ticker.replace("P:", "O:");
						if(OptionsRegionalQTMessageQueue.getInstance().isSubscribedSymbol(opticker))
							return;
						subscribe(ticker);
					}
					else
