package ntp.optionsregional.snap;
...
public class OptionsRegionalSnapController {

	private UltraChan[] channelArray;
	private UltraChan[] preChannelArray;
	private long count = 0;
	private static volatile OptionsRegionalSnapController instance = new OptionsRegionalSnapController();
	private OptionsRegionalUtility utility = OptionsRegionalUtility.getInstance();

	private OptionsRegionalSnapController()
	{
		...
		try{ preIP = CPDProperty.getInstance().getProperty("PRE_SNAP_IP"); }
		catch(Exception e)
		{
		...
		try{ prePort = Integer.parseInt(CPDProperty.getInstance().getProperty("PRE_SNAP_PORT")); }
		catch(Exception e)
		{
		...
		for(int i = 0; i < snapChannelCount; i++)
		{
			...
			chan.Start();
			NTPLogger.connectChannel(name, preIP);
		}
	}

	public static OptionsRegionalSnapController getInstance()
	{
		return instance;
	}
	...
	public Map<String,Image> getSyncSnapData(HashSet<String> resubscribeSet,boolean isPre){
				...
				for (int index = 0; index < tickCount; index+=1000)
				{
					...
					for(int j =0;j<tickerSnaps.length;j++)
					{
						...
						else if(mt== UltraChan._mtDEAD)
						{
							...
							{
								NTPLogger.dead(tkr, "OpRegSyncMulti");
								if(!isPre)
									OptionsRegionalQTMessageQueue.getInstance().getIncorrectTickerMap().put(tkr, tkr);
							}
						}
						else
							NTPLogger.unknown(tickerSnaps[j].tkr(), "OpRegSyncMulti", mt);
					}
					tickerSnaps = null;
				}
