...
public class NbSubscriptionChannel extends UltraChan {
	...
	private ConcurrentHashMap<String, String> imageTickerMap = NbQTMessageQueue.getInstance().getImageTickerMap();
	private boolean isExtendedEval = true;
	private String exchgFilter[];
	private final String NASDAQ_BASIC_SUFFIX = "/T";
	private String tickerSuffix = NbQTMessageQueue.getInstance().getSymbolSuffix();

	public NbSubscriptionChannel(String name) {
	...
	public void OnImage(String StreamName, Image img) {
		...
		if (ticker.toUpperCase().endsWith(NASDAQ_BASIC_SUFFIX)) {
			...
			NbQTMessageQueue.getInstance().getmQueue().add(bean.getTicker(), bean);
			return;
		} else {
			...
			if (img._halted == 0x01) {
				...
				NTPLogger
						.info("Image NB Ticker: " + ticker + " HLT: " + false + " " + Integer.toHexString(img._halted));
			}
	...
	private QTCPDMessageBean getCachedBean(String ticker) {
		Map<String, QTCPDMessageBean> qtMap = NbQTMessageQueue.getInstance().getSubsData();
