...
public class NYSEMarketStatUpdater {

	private static NYSEMarketStatUpdater nyseUpdater;
	...
	private NYSEMarketStatUpdater()
	{
		NYSEMarketStatStreamer streamer  =  new NYSEMarketStatStreamer();
		Thread t = new Thread(streamer);
		t.start();
	}

	public static NYSEMarketStatUpdater getInstance()
	{
		if(nyseUpdater == null)
			nyseUpdater = new NYSEMarketStatUpdater();
		return nyseUpdater;
	}
	...
	public boolean isNYSETicker(String ticker)
	{
		if(tickList.contains(ticker))
			return true;
		return false;
...
//		if(vol == 0)
//			return;
		//assuming that ticker is of a particular exchange, says NYSE
...
//		System.out.println(ticker + " " + tickStatus);
	...
	private class NYSEMarketStatStreamer implements Runnable{
		...
		private QTCPDMessageBean totalVol1Bean = new QTCPDMessageBean();
		...
		public NYSEMarketStatStreamer()
		{
			initializeBean(upVolBean, MarketStatTickerManager.SYMBOL_NYSE_UP_VOLUME);
			initializeBean(downVolBean, MarketStatTickerManager.SYMBOL_NYSE_DOWN_VOLUME);
			initializeBean(totalVolBean, MarketStatTickerManager.SYMBOL_NYSE_TOTAL_VOLUME);
			initializeBean(totalVol1Bean, MarketStatTickerManager.SYMBOL_NYSE_TOTAL_VOLUME_1);
			initializeBean(uncgIsueBean, MarketStatTickerManager.SYMBOL_NYSE_UNCHANGED_ISSUE);
			initializeBean(advIsueBean, MarketStatTickerManager.SYMBOL_NYSE_ADVANCING_ISSUE);
			initializeBean(declIsueBean, MarketStatTickerManager.SYMBOL_NYSE_DECLINING_ISSUE);
			initializeBean(udriBean, MarketStatTickerManager.SYMBOL_NYSE_ISSUES_UP_DOWN_RATIO);
			initializeBean(sttiBean, MarketStatTickerManager.SYMBOL_NYSE_SHORT_TERM_TRADE_INDEX);
				...
				try {
					if(isUpdated)
					{
						...
						double totalVol = upVol + downVol + uncgVol;
						{
							...
							streamBean(totalVol1Bean, totalVol);
						}
