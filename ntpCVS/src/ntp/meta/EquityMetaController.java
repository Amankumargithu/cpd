...
public class EquityMetaController {
	...
	public static void main(String[] args) {
		...
		try
		{
			...
			String channels = prop.getProperty("CHANNELS");
			String[] channelArray ;
			if(channels == null)
			{
				NTPLogger.missingProperty("CHANNELS");
				channelArray = new String[]{"0"};
				NTPLogger.defaultSetting("Channels", "0");
			}
			else
			...
			{
				NTPLogger.missingProperty("MC_LOCATE_FILE");//mcloc_equity.txt
				marketCenterLocateFile = "mcloc_equity";
			...
			{
				NTPLogger.missingProperty("METADATA_FILE");//meta_equity.txt
				metaDataFile = "meta_equity";
			...
			{
				NTPLogger.missingProperty("OUTPUT_META_FILE");//outputMetaEquity.csv
				outputMetaFile = "outputMetaEquity";
			...
			{
				NTPLogger.missingProperty("OUTPUT_MCLOC_FILE");//outputMclocEquity.csv
				outputMclocFile = "outputMclocEquity";
			...
			{
				NTPLogger.missingProperty("OUTPUT_PRIMARY_MC_FILE");//outputPrimaryMcEquity.csv
				outputPrimaryMCFile = "outputPrimaryMcEquity";
			...
			for(int i=0; i< channelArray.length; i++){
				NTPLogger.info("Processing Channel No. "+i);
				...
				fileName = rootDir + "/" + parseDate + "/" + marketCenterLocateFile+"_"+channelArray[i]+".txt";
				NTPLogger.info("Reading file " + fileName);
				...
				if(i==2){
					NTPLogger.info("locate code map is : "+locateCodeMap);
				}
				fileName = rootDir + "/" + parseDate + "/" + outputMclocFile+"_"+channelArray[i]+".csv" ;
				...
				fileName = rootDir + "/" + parseDate + "/" + metaDataFile+"_"+channelArray[i]+".txt";
				NTPLogger.info("Reading file " + fileName);
				...
				if(bufferedReader != null)
				{
					...
					while( (line = bufferedReader.readLine()) != null)
					{
						...
						if(!(values.length > 1)){
							NTPLogger.info("Protocol is not present for line : "+line);
							continue;
						}
						...
						if(bean == null)
						{
							...
							if(channelArray[i].equals("2")){
								bean.setPrimaryMarket("T");
								primaryMarketMap.put(ticker, bean.getPrimaryMarket());
							}
//							if(channelArray[i].equals("17")){
//								bean.setPrimaryMarket("BB");
//								primaryMarketMap.put(ticker, bean.getPrimaryMarket());
//							}
						...
						try
						{
							...
							case "3":
								if(arr.length > 9){
									...
									NTPLogger.info("Null(3) for line : "+line);
								}
								break; 	
							case "4":
								if(arr.length > 9)
									...
									NTPLogger.info("Null(4) for line : "+line);
								break;
							case "5":
						...
						{
							NTPLogger.info(meta + "_line is :"+line+" _ " + e.getMessage());
							e.printStackTrace();
						}
						catch (Error e) {
							NTPLogger.info(meta + "_line is :"+line+" _ " + e.getMessage());
				...
				fileName = rootDir + "/" + parseDate + "/" + outputMetaFile+"_"+channelArray[i]+".csv";
				NTPLogger.info("Writing file " + fileName);
				...
				fileName = rootDir + "/" + parseDate + "/" + outputPrimaryMCFile+"_"+channelArray[i]+".csv";
				NTPLogger.info("Writing file " + fileName);
				fileWriter = new CsvWriter(fileName);
