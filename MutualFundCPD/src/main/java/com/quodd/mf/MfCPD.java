...
import com.quodd.mf.util.MfMetaDataUtility;

public class MfCPD extends CPD {
	public static final MfCache datacache = new MfCache();
	private final MfMetaDataUtility metaUtility = new MfMetaDataUtility();
	...
	public MfCPD() {
		...
		if (datacache.isLegacyDistribution()) {
			...
			JMSPropertyBean legacyJmsProperty = new JMSPropertyBean(lqos, lclientId, ltopicName, lconflationTime,
					lsurgeThreshold);
			legacyDispacther.startDispatcher("MutualFund", datacache.getLegacymQueue(), legacyJmsProperty);
		}
		if (datacache.isWebSocketDistribution()) {
			...
			tradeDispacther.startDispatcher("MutualFund", datacache.getTrademQueue(), "trade", writeFile,
					datacache.getCachedTradeData(), jmsProperty);
		}

		this.numberofChannels = cpdProperties.getIntProperty("NO_OF_CHANNELS", 6);
		this.apiPort = cpdProperties.getIntProperty("API_PORT", 8287);
	...
	private void startProcess() {
		...
		path("mfcpd", () -> {
			...
			path("/snap", () -> {
				get("", MfDataController.getData, gson::toJson);
				get("/dump/:dataType", MfDataController.getDataByServiceId);
				post("/list", MfDataController.getDataList, gson::toJson);
			});
			path("/subscribe", () -> get("", MfDataController.subscribe, gson::toJson));
			path("/unsubscribe", () -> get("", MfDataController.unsubscribe, gson::toJson));
			path("/list", () -> {
				get("/meta/:serviceId", MfDataController.getTickerServices, gson::toJson);
				get("/image", MfDataController.getImageTickers, gson::toJson);
				get("/pending", MfDataController.getPendingTickers, gson::toJson);
				get("/root", MfDataController.getRootSymbols, gson::toJson);
			});
			path("/filewriter", () -> {
				get("/start/:dataType", MfDataController.startFileWriter, gson::toJson);
				get("/stop/:dataType", MfDataController.stopFileWriter, gson::toJson);
			});
		});
		...
		for (int i = 0; i <= this.numberofChannels; i++) {
			MfSubsChannel channel = new MfSubsChannel(this.ip, this.port, "MfChan" + i);
