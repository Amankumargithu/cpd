...
public class BondsDataController {
	...
	public static final Route getTickerServices = (Request request, Response response) -> {
		...
		if (datacache.getTradeProtocol() == id || datacache.getSnapTradeProtocol() == id
				|| datacache.getQuoteProtocol() == id || datacache.getSnapQuoteProtocol() == id)
			return datacache.getMetaTickerSet();
		return new HashSet<>();
	};
