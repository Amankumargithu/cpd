...
public class EmailManager {
	...
	public void send(String to, String cc, String bcc, String from,
			String fromPersonalName, String smtp, String subject,
			String msg_body, boolean isHtml, boolean debug, String[] files)
			throws MessagingException {

		// check if we receieved a valid request...
		if (isHtml && files != null && files.length > 0) {
			String error = "File attachments can only be processed "
					+ "using a mime type of \"text/plain\" NOT "
					+ "\"text/html\".";
		...
		try {
			...
			try {
				//System.out.println("From email address personal name : "+fromPersonalName);
				msg.setFrom(new InternetAddress(from, fromPersonalName));
			}
			catch (Exception pne) {
				System.out.println("Unable to set personal name in the from email address - "+pne.getMessage());
				pne.printStackTrace();
	...
	public void send(String to, String cc, String bcc, String from,
			String smtp, String subject, String msg_body, boolean isHtml,
			boolean debug, String[] files) throws MessagingException {
	 ...
	 */
	private InternetAddress[] extractRecipients(String to)
			throws MessagingException {
	...
	/**
	 * This method will allow the client to send plain text ("text/plain") and
	 * file attachments.
	 */
	private Multipart processMultipart(String msg_body, String[] files)
			throws MessagingException {
