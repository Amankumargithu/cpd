...
import com.quodd.index.util.GidsMetaDataUtility;

public class GidsCPD extends CPD {
	public static final GidsCache datacache = new GidsCache();
	private final GidsMetaDataUtility metaUtility = new GidsMetaDataUtility();
	...
	public GidsCPD() {
		...
		if (datacache.isWebSocketDistribution()) {
			...
			tradeDispacther.startDispatcher("Gids", datacache.getTrademQueue(), "trade", writeFile,
		...
		if (datacache.isLegacyDistribution()) {
			...
			JMSPropertyBean legacyJmsProperty = new JMSPropertyBean(lqos, lclientId, ltopicName, lconflationTime,
					lsurgeThreshold);
			legacyDispacther.startDispatcher("Gids", datacache.getLegacymQueue(), legacyJmsProperty);
		}
	...
	private void startProcess() {
		...
		path("indcpd", () -> {
			...
			path("/snap", () -> {
				get("", GidsDataController.getData, gson::toJson);
				get("/dump/:dataType", GidsDataController.getDataByServiceId);
				post("/list", GidsDataController.getDataList, gson::toJson);
			});
			path("/subscribe", () -> get("", GidsDataController.subscribe, gson::toJson));

			path("/list", () -> {
				get("/meta/:serviceId", GidsDataController.getTickerServices, gson::toJson);
				get("/image", GidsDataController.getImageTickers, gson::toJson);
				get("/pending", GidsDataController.getPendingTickers, gson::toJson);
				get("/root", GidsDataController.getRootSymbols, gson::toJson);
			});
			path("/unsubscribe", () -> {
				get("", GidsDataController.unsubscribe, gson::toJson);
			});
			path("/filewriter", () -> {
				get("/start/:dataType", GidsDataController.startFileWriter, gson::toJson);
				get("/stop/:dataType", GidsDataController.stopFileWriter, gson::toJson);
			});
		});
		...
		for (int i = 0; i <= this.numberofChannels; i++) {
			GidsSubsChannel channel = new GidsSubsChannel(this.ip, this.port, "IdxChan" + i);
