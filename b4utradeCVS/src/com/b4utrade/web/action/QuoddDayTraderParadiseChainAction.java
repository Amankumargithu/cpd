...
public class QuoddDayTraderParadiseChainAction extends B4UTradeDefaultAction {

	static Log log = LogFactory.getLog(QuoddDayTraderParadiseChainAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			if (!("daytraderparadise".equals(username) && "ondemanddaytraderparadise".equals(password))) {
						...
						if (od != null) {
							try {
								...
								if (temphash != null) {
									...
									log.warn(
											"QuoddDayTraderParadiseChainAction.getOptionDataList - unable to obtain Future Option Data Handler from object pool!");
								}
								break;
							} catch (Exception e) {
								log.warn(
										"QuoddDayTraderParadiseChainAction.execute - exception while connecting OptionDataHandler");
								temphash = od.getOptionChain(tickerName.toUpperCase());
							}
						} else {
							log.error(
									"QuoddDayTraderParadiseChainAction.execute - OptionData remote interface object is null.");
							throw new Exception("Option Data Remote interface is null.");
						}
					} else
						log.error("QuoddDayTraderParadiseChainAction.execute - OptionDataHandler is null.");
					break;
				} catch (Exception exc) {
					log.warn(
							"QuoddDayTraderParadiseChainAction.execute - Exception encountered while trying to get Option Data.",
							exc);
					if (odh != null)
						odh.init();
				} finally {
			...
			try (ServletOutputStream sos = response.getOutputStream();) {
				...
				log.error("QuoddDayTraderParadiseChainAction.execute() : encountered exception. ", e);
			}
		} catch (Exception e) {
			String msg = "QuoddDayTraderParadiseChainAction Unable to retrieve data from the ejbCall.";
			log.error(msg, e);
		}
		return null;
	}
