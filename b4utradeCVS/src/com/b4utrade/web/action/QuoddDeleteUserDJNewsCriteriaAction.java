...
public class QuoddDeleteUserDJNewsCriteriaAction extends B4UTradeDefaultAction {

	static Log log = LogFactory.getLog(QuoddDeleteUserDJNewsCriteriaAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			if (userId <= 0) {
				log.error("QuoddDeleteUserDJNewsCriteriaAction.execute: unable to find USER_ID from request cookie.");
				statusbean.setOperationFailed();
			} else {
				String dataInputObj = request.getParameter("NEWS_SEARCH_BEAN");
				log.info("QuoddDeleteUserDJNewsCriteriaAction.execute() : News search criteria = " + dataInputObj
						+ " id: " + userId);
				...
				try (ByteArrayInputStream bais = new ByteArrayInputStream(dataInputObj.getBytes());
					...
					boolean ok = NewsSelector.deleteDJNewsCriteria(userId, criteriaBean);
					if (ok)
						statusbean.setOperationSuccessful();
					else
						statusbean.setOperationFailed();
				} catch (Exception e) {
					log.error("QuoddDeleteUserDJNewsCriteriaAction.execute: encountered an error for user=" + userId,
							e);
					statusbean.setOperationFailed();
				}
			}
			...
			try (ServletOutputStream sos = response.getOutputStream();) {
				...
				log.error("QuoddDeleteUserDJNewsCriteriaAction.execute(): servlet output stream exception. ", e);
			}
		} catch (Exception e) {
			String msg = "Unable to delete data.";
			log.error("QuoddDeleteUserDJNewsCriteriaAction.execute(): " + msg, e);
		}
		return null;
	}
}
