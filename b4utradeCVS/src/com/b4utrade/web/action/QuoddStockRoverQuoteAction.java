...
public class QuoddStockRoverQuoteAction extends B4UTradeDefaultAction {

	static Log log = LogFactory.getLog(QuoddStockRoverQuoteAction.class);
	private static Gson gson = new Gson();
	private static Type t = new TypeToken<ArrayList<HashMap>>() {
	}.getType();
	private static HashSet<String> allowedProtocols = new HashSet<>();
	static {
		allowedProtocols.add("14");
		allowedProtocols.add("34");
		allowedProtocols.add("9");
		allowedProtocols.add("8");
		allowedProtocols.add("186");
		allowedProtocols.add("187");
		allowedProtocols.add("190");
		allowedProtocols.add("191");
		allowedProtocols.add("192");
		allowedProtocols.add("193");
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try (InputStream in = request.getInputStream()) {
			if (in != null) {
				...
				log.info("QuoddStockRoverQuoteAction Request " + params);
			} else {
				log.warn("QuoddStockRoverQuoteAction parameters are is null.");
			}
		}
	...
	private static List<EquityMessageBean> getDelayedSnapData(List<String> modifiesTickers, String snapUrl) {
		...
		try {
			...
			log.info("QuoddStockRoverQuoteAction Delayed Snap url is : " + updatedUrl.toString());
			...
			try (ObjectInputStream objectStream = new ObjectInputStream(urlConnection.getInputStream());) {
				...
				log.error("QuoddStockRoverQuoteAction.getDelayedSnapData exception " + e.getMessage(), e);
			}
			if (o != null) {
