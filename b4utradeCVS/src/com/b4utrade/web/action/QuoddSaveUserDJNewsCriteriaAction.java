...
/**
 * Saves Dow Jones News search criteria for user.
 */

public class QuoddSaveUserDJNewsCriteriaAction extends B4UTradeDefaultAction {

	static Log log = LogFactory.getLog(QuoddSaveUserDJNewsCriteriaAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			if (userId <= 0) {
				log.error("QuoddSaveUserDJNewsCriteriaAction.execute: unable to find USER_ID from request cookie.");
				statusbean.setOperationFailed();
			} else {
				String dataInputObj = request.getParameter("NEWS_SEARCH_BEAN");
				log.info("QuoddSaveUserDJNewsCriteriaAction.execute() : News search criteria = " + dataInputObj
						+ " id: " + userId);
				...
				try (ByteArrayInputStream bais = new ByteArrayInputStream(dataInputObj.getBytes());
					...
					if (criteriaBean.getCategoryV() != null) {
						NewsCriteriaDetailBean ncdbean = null;
						// We are receiving only single bean in this vector
						for (Enumeration en = criteriaBean.getCategoryV().elements(); en.hasMoreElements();) {
							ncdbean = (NewsCriteriaDetailBean) en.nextElement();
							ncdbean.setCriteriaListId(1);
							NewsSelector.deleteDJNewsCriteria(userId, criteriaBean);
							if (NewsSelector.saveDjNewsCriteria(userId, criteriaBean, ncdbean))
								saveflag = true;
						}
					}
					if (saveflag)
					...
					log.error("QuoddSaveUserDJNewsCriteriaAction.execute: encountered an error for user=" + userId, e);
					statusbean.setOperationFailed();
				}
			}
			...
			try (ServletOutputStream sos = response.getOutputStream();) {
				...
				log.error("QuoddSaveUserDJNewsCriteriaAction.execute() : servlet output stream exception. ", e);
			}
		} catch (Exception e) {
			String msg = "Unable to save data.";
			log.error("QuoddSaveUserDJNewsCriteriaAction.execute(): " + msg, e);
		}
		return null;
	}
}
