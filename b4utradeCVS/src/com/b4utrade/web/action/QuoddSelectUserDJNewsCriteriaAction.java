...
public class QuoddSelectUserDJNewsCriteriaAction extends B4UTradeDefaultAction {

	static Log log = LogFactory.getLog(QuoddSelectUserDJNewsCriteriaAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			if (userId <= 0) {
				log.error("QuoddSelectUserDJNewsCriteriaAction.execute: unable to find USER_ID from request cookie.");
			} else {
				Vector<NewsCriteriaBean> resultList = NewsSelector.getDJNewsCriteria(userId);
				...
				try (ServletOutputStream sos = response.getOutputStream();) {
					...
					log.error("QuoddSelectUserDJNewsCriteriaAction.execute() : encountered exception. ", e);
				}
			}
		} catch (Exception e) {
			String msg = "QuoddSelectUserDJNewsCriteriaAction.execute() : Unable to retrieve data.";
			log.error(msg, e);
		}
		return null;
	}
