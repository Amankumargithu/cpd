...
public class CompressedLZWTSQStreamer implements IStreamer, Runnable {
   ...
   public CompressedLZWTSQStreamer() {}
   ...
   public void init(MessageQueue quoteQueue, MessageQueue newsQueue, InputStream inputStream,
      ...
      SystemOutLogger.debug("Compressed TSQ Streamer - init()");
   ...
   public void run() {
      ...
      try {
         ...
         SystemOutLogger.error("TSQ Streamer - Unable to connect to URL: "+networkConfiguration.getStreamingURL());
         e.printStackTrace();
         return;
      }
      finally {
      ...
      while ( doRun ) {
         ...
         try {
            ...
            if ( (curTime - beginTime) > 1000) {
               beginTime = curTime;
               SystemOutLogger.debug("Comp TSQ Streamer - Number of messages processed: "+msgCount);
               SystemOutLogger.debug("Comp TSQ Streamer - Queue size: "+quoteQueue.getSize());
               msgCount = 0l;
            }
...
/*            System.out.println("Ticker: " + bean.getTicker()
                             + " LTV: " +bean.getTradeSize() + " LTP: " +bean.getTradePrice()
                             + "Bid Price: " +bean.getBidPrice() + "Bid size: " +bean.getBidSize()
                             + "ask Price: " +bean.getAskPrice() + "ask size: " +bean.getAskSize() );
*/
                     quoteQueue.add(bean);
                     msgCount++;
                  }
            ...
            SystemOutLogger.debug("Comp TSQ Streamer - Exception caught during input stream read." + ticker);
            ioe.printStackTrace();
            return;
         }
      }
