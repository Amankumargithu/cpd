...
public class OptimizedOptionStreamer implements IStreamer, Runnable {
   ...
   public OptimizedOptionStreamer() {}
   ...
   public void run() {
      ...
      try {
         ...
         SystemOutLogger.error("Opt Option Streamer - Unable to connect to URL: "+networkConfiguration.getStreamingURL());
         e.printStackTrace();
         return;
      }
      finally {
      ...
      while ( doRun ) {
         ...
         try {
            ...
            if ( (curTime - beginTime) > 1000) {
               beginTime = curTime;
               SystemOutLogger.debug("Opt Option Streamer - Number of messages processed: "+msgCount);
               SystemOutLogger.debug("Opt Option Streamer - Queue size: "+queue.getSize());
               msgCount = 0l;
            }
            ...
            for (int i=0; i<bytesRead; i++) {
               if (response[i] == terminatorByte) {
                  ...
                  if (resultsCache == null) {
                     SystemOutLogger.debug("Opt Option Streamer - unable to obtain network configuration results hash!");
                     baos.reset();
                     continue;
                  }

                  Hashtable map = messageInflator.parseKeyValues(baos);
                  QTMessageBean qtmb = (QTMessageBean)resultsCache.get(map.get(QTMessageKeys.TICKER));

                  qtmb = messageInflator.merge(map, qtmb);
                  resultsCache.put(qtmb.getTicker(),qtmb);
			...
			ioe.printStackTrace();
         SystemOutLogger.error("Opt Option Streamer - Exception caught during input stream read.");
            return;
         }
      }
