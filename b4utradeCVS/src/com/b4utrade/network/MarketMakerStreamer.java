...
public class MarketMakerStreamer implements IStreamer, Runnable {
   ...
   public void run() {
      ...
      try {
         ...
         SystemOutLogger.error("Market Maker Streamer - Unable to connect to URL: "+networkConfiguration.getStreamingURL());
         e.printStackTrace();
         return;
      }
      finally {
      ...
      while ( doRun ) {
         ...
         try {
            ...
            if ( (curTime - beginTime) > 1000) {
               beginTime = curTime;
               SystemOutLogger.debug("MarketMaker Streamer - Number of messages processed: "+msgCount);
               SystemOutLogger.debug("MarketMaker Streamer - Queue size: "+queue.getSize());
               msgCount = 0l;
            }

            for (int i=0; i<bytesRead; i++) {
               if (response[i] == terminatorByte) {
				  //SystemOutLogger.debug("level II string: " + new String(baos.toByteArray()));
                  MM2MessageBean mm2mb = MM2MessageBean.convertMM2MessageBean(baos.toByteArray());
                  if (mm2mb.isParseOk()) {
                    mm2mb.init();
                    SystemOutLogger.debug("level II ticker in network streamer: " + mm2mb.getTicker());
            ...
            SystemOutLogger.error("MarketMaker Streamer - Exception caught during input stream read.");
            return;
         }
      }
