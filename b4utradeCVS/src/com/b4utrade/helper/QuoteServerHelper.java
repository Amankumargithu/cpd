		...
		while (numTries <= 1) {
			try {
				...
				if (odh != null) {
					...
					Logger.log(
							"QuoteServerHelper.getFutureOptionDataList - unable to obtain Future Option Data Handler from object pool!");
				}
				break;
			} catch (Exception exc) {
			...
			try {
				...
				if (odh != null) {
					...
					if (od != null) {
						...
						Logger.log("getFutureDataList - OptionData is null.");
						throw new Exception("Futures - OptionData Remote interface is null.");
					}
				} else {
					Logger.log(
							"QuoteServerHelper.getFutureDataList - unable to obtain OptionData Handler from object pool");
				}
				break;
			} catch (Exception exc) {
				Logger.log("QuoteServerHelper - Exception encountered while trying to get Futures Data.", exc);
				if (odh != null) {
					odh.init();
				}
			} finally {
