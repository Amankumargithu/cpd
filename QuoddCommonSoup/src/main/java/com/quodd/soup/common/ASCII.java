...
public class ASCII {
	 ...
	 *
	 * <p>
	 * If the length of the string is smaller than the length of the array, the
	 * space character is used to fill the trailing bytes.
	 * </p>
	 *
	 * @param bytes a byte array
	 * @param s     a string
	 * @throws IndexOutOfBoundsException if the length of the string is larger than
	 ...
	 * <p>
	 * If the length of the string is smaller than the length of the array, the
	 * space character is used to fill the leading bytes.
	 * </p>
	 *
	 * @param bytes a byte array
	 * @param s     a string
	...
	public static void putRight(byte[] bytes, CharSequence s) {
		...
		for (; i < bytes.length - s.length(); i++)
			bytes[i] = (byte) ' ';
		for (int j = 0; j < s.length() && j < bytes.length; i++, j++)
