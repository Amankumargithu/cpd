...
public class IndicesSubsChannel extends CPDChannel {

	private final ConcurrentMap<String, String> imageTickerMap = datacache.getImageTickerMap();

	public IndicesSubsChannel(String ip, int port, String name) {
	...
	public void OnImage(String streamName, Image image) {
		try {
			...
			if (datacache.isLegacyDistribution()) {
				...
				legacyData.put(CPDConstants.extTradeExh, " ");
			...
			int serviceCode = datacache.getServiceCode(ticker);
			changedData.put(CPDConstants.tapeSet, datacache.getTapeset(serviceCode));
			changedData.put(CPDConstants.protocolId, serviceCode);
			...
			if (datacache.isLegacyDistribution()) {
				...
				legacyData.put(CPDConstants.tradeDate, CPDUtility.processDate(image.JavaTime(image._tUpd)));
			...
			String key = ticker + "|" + datacache.getServiceCode(ticker);
	...
	public void OnUpdate(String streamName, IDXValue val) {
		try {
			...
			String ke = tkr + "|" + datacache.getServiceCode(tkr);
			...
			changedData.put(CPDConstants.protocolId, datacache.getServiceCode(tkr));
...
//				legacyData.put(CPDConstants.protocolId, datacache.getServiceCode(tkr));
			...
			String key = tkr + "|" + datacache.getServiceCode(tkr);
			datacache.getTrademQueue().putAll(key, changedData);
