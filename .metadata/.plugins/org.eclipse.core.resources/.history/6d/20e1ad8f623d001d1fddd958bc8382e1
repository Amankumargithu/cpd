				...
				try {
					if (odh != null) {
						...
						if (od != null) {
							...
							log.error("QuoteAsXMLDataFormatter.execute - FutureData remote interface object is null.");
							throw new Exception("Future Data Remote interface is null.");
						}
					} else
						log.error("QuoteAsXMLDataFormatter.execute - FutureDataHandler is null.");
					break;
				} catch (Exception exc) {
					Logger.log("QuoteAsXMLDataFormatter.execute - Exception encountered while trying to get Future Data.",exc);
					numTries++;
					if (odh != null)
						odh.init();
				}
