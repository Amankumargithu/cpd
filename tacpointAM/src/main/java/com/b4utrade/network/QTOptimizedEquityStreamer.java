...
public class  QTOptimizedEquityStreamer implements IStreamer, Runnable {
   ...
   public QTOptimizedEquityStreamer() {}
               ...
               if (response[i] == terminatorByte) {
                  ...
                  
                  {
                     ...
                     if((qtBean.getLastPrice() <0.00001d) && (symbol.indexOf("TACPOINT") < 0))
                       SystemOutLogger.debug("the last price is 0.0, for symbol " + symbol + " the string from streamer: " + new String(baos.toByteArray()));
                     //QTMessageBean newBean = new QTMessageBean();

                     if (qtBean != null) {
						/*if (qtBean.getSystemTicker().indexOf(".FT") > 0){
						  System.out.println("Future Ticker: " + qtBean.getSystemTicker() + " volumn: " +qtBean.getVolume() + " last Price " + qtBean.getLastPrice());
					     }
					    */
                        resultsCache.put(qtBean.getSystemTicker(),qtBean);
                        QTMessageBean newBean = QTMessageBean.getQTMessageBean(qtBean.toByteArray());
                       
                        /*
                         
                        //set the the market center info also - now part of the QTMessageBean
                         
                        newBean.setExchangeCode(qtBean.getExchangeCode());

                        newBean.setAskExchangeCode(qtBean.getAskExchangeCode());
                        newBean.setBidExchangeCode(qtBean.getBidExchangeCode());
                        newBean.setMarketCenter(qtBean.getMarketCenter());
                        newBean.setVWAP(qtBean.getVWAP());
                        */

                        quoteQueue.add(newBean);
