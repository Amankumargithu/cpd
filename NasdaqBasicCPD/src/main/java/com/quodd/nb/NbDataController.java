...
public class NbDataController {
	...
	public static final Route getNasdaqBasicSymbols = (Request request, Response response) -> datacache
			.getNbMetaUcTickerSet();
	...
	public static final Route getTickerServices = (Request request, Response response) -> {
		...
		if (datacache.getTradeProtocol() == id || datacache.getQuoteProtocol() == id
				|| datacache.getSnapTradeProtocol() == id || datacache.getSnapQuoteProtocol() == id) {
			return datacache.getNbMetaQuoddTickerSet();
		}
		return new HashSet<>();
	};
